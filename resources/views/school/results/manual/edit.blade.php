    ...
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Update student result for {{ $school->name }}.
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('school.results.manual.update', $studentResult) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="student_id"
                                ...
                                @foreach ($students as $student)
                                    <option value="{{ $student->id }}" @selected(old('student_id', $studentResult->student_id) == $student->id)>
                        ...
                        <div>
                            ...
                            <select name="subject_id"
                                ...
                                @foreach ($subjects as $subject)
                                    <option value="{{ $subject->id }}" @selected(old('subject_id', $studentResult->subject_id) == $subject->id)>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="academic_session_id"
                                ...
                                @foreach ($academicSessions as $academicSession)
                                    <option value="{{ $academicSession->id }}" @selected(old('academic_session_id', $studentResult->academic_session_id) == $academicSession->id)>
                        ...
                        <div>
                            ...
                            <select name="term_id"
                                ...
                                @foreach ($terms as $term)
                                    <option value="{{ $term->id }}" @selected(old('term_id', $studentResult->term_id) == $term->id)>
                    ...
                    <div class="grid gap-6 sm:grid-cols-3">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" max="60" name="exam_score" value="{{ old('exam_score', $studentResult->exam_score) }}"
                                   class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                            @error('exam_score')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                                <option value="draft" @selected(old('status', $studentResult->status) === 'draft')>Draft</option>
                                <option value="reviewed" @selected(old('status', $studentResult->status) === 'reviewed')>Reviewed</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
