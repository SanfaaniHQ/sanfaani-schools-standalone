                ...
                <form method="POST" action="{{ route('school.subjects.store') }}" class="space-y-6">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <select name="assignment_type"
                                ...
                                @foreach (\App\Models\ClassSubjectAssignment::TYPES as $type)
                                    <option value="{{ $type }}" @selected(old('assignment_type', 'core') === $type)>{{ ucfirst($type) }}</option>
                                @endforeach
                            </select>
                        ...
                        <label class="mt-6 flex items-center gap-3 rounded-xl border border-gray-200 p-4 text-sm text-gray-700">
                            <input type="checkbox" name="is_elective" value="1" @checked(old('is_elective')) class="rounded border-gray-300">
                            Elective subject
                        </label>
                    </div>
