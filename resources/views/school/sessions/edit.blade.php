    ...
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Update academic session for {{ $school->name }}.
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('school.sessions.update', $academicSession) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="responsive-form-grid gap-6">
                        <div>
                            ...
                            <input type="date" name="starts_at" value="{{ old('starts_at', $academicSession->starts_at?->format('Y-m-d')) }}"
                                   class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                            @error('starts_at')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" name="ends_at" value="{{ old('ends_at', $academicSession->ends_at?->format('Y-m-d')) }}"
                                   class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                            @error('ends_at')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <select name="status"
                                class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                            <option value="active" @selected(old('status', $academicSession->status) === 'active')>Active</option>
                            <option value="inactive" @selected(old('status', $academicSession->status) === 'inactive')>Inactive</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" value="1"
                               ...
                               @checked(old('is_active', $academicSession->is_active))>
