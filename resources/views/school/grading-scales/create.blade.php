    ...
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Create a custom grading rule for {{ $school->name }}.
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('school.grading-scales.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text"
                               name="name"
                               value="{{ old('name', 'Default Grading Scale') }}"
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="remark"
                                   value="{{ old('remark') }}"
                                   placeholder="Excellent, Credit, Striving"
                                   class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                            @error('remark')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid gap-6 sm:grid-cols-3">
                        <div>
                            ...
                            <select name="is_pass"
                                    class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                                <option value="1" @selected(old('is_pass', '1') === '1')>Pass</option>
                                <option value="0" @selected(old('is_pass') === '0')>Not Pass</option>
                            </select>
                            @error('is_pass')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
