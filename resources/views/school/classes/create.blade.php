    ...
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Create a class for {{ $school->name }}.
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('school.classes.store') }}" class="space-y-6">
                    @csrf

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Class Name</label>
                        <input type="text" name="name" value="{{ old('name') }}"
                               placeholder="Example: JSS 1, JSS 2, SS 1"
                    ...
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code') }}"
                               placeholder="Example: JSS1, SS2-SCI"
                               class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                        <p class="mt-1 text-xs text-gray-500">Optional. Codes are helpful for uploads and quick filters.</p>
                        @error('code')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="section" value="{{ old('section') }}"
                               placeholder="Example: A, B, Science, Commercial"
                               class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                        <p class="mt-1 text-xs text-gray-500">
                            Optional. Use this if the school separates classes into arms or streams.
                        </p>
                        @error('section')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
