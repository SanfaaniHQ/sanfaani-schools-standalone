            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm">
                ...
                @if ($academicSessions->isEmpty())
                    <div class="rounded-xl bg-yellow-50 p-4 text-sm text-yellow-800">
                        You need to create an academic session before adding terms.
                    </div>
                ...
                @else
                    <form method="POST" action="{{ route('school.terms.store') }}" class="space-y-6">
                        @csrf
                            ...
                            <select name="academic_session_id"
                                ...
                                @foreach ($academicSessions as $academicSession)
                                    <option value="{{ $academicSession->id }}"
                                            @selected(old('academic_session_id') == $academicSession->id)>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}"
                                   placeholder="Example: First Term, Second Term"
                                   class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                            @error('name')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
