<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold leading-tight text-gray-900">School Audit Logs</h2>
            <p class="mt-1 text-sm text-gray-500">Review isolated activity for {{ $school->name }}.</p>
        </div>
    </x-slot>

    <div class="py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <form method="GET" action="{{ route('school.audit-logs.index') }}" class="mb-6 grid gap-3 rounded-2xl bg-white p-4 shadow-sm md:grid-cols-4">
                ...
                <div class="flex flex-wrap gap-2 md:col-span-4">
                    ...
                    <a href="{{ route('school.audit-logs.index') }}" class="rounded-xl border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700">Clear filter</a>
                    <a href="{{ route('school.audit-logs.export', request()->query()) }}" class="rounded-xl border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700">Export CSV</a>
                </div>
            </form>

            <div class="overflow-hidden rounded-2xl bg-white shadow-sm">
                <div class="safe-scroll-x rounded-none border-0 shadow-none" role="region" aria-label="School audit logs" tabindex="0">
                    <table class="min-w-full divide-y divide-gray-100 text-sm">
                        <thead class="bg-gray-50 text-xs uppercase tracking-normal text-gray-500">
                            <tr>
                            ...
                            @forelse ($logs as $log)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-xs text-gray-500">
                                        @if ($log->payload || $log->metadata || $log->old_values || $log->new_values)
                                            <details>
                                                <summary class="cursor-pointer font-medium text-gray-700">View readable values</summary>
                                                <pre class="mt-2 max-h-40 overflow-auto rounded-lg bg-gray-50 p-3">{{ json_encode(['payload' => $log->payload, 'metadata' => $log->metadata, 'old' => $log->old_values, 'new' => $log->new_values], JSON_PRETTY_PRINT) }}</pre>
                                            </details>
                                        @else
                                            No value changes
                                        @endif
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-12 text-center">
                                        <p class="text-sm font-medium text-gray-900">No school audit logs found.</p>
                                        <p class="mt-1 text-sm text-gray-500">Activity will appear here as school workflows run.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
