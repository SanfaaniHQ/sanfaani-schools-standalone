@php
    $emailMeta = $mailMetadata ?? [];
    $roleLabel = data_get($emailMeta, 'role_label') ?? ucwords(str_replace('_', ' ', (string) data_get($emailMeta, 'role', 'staff')));
@endphp

@component('emails.layout', ['subject' => $subjectLine ?? 'Staff Notification', 'school' => $school ?? null])
    <p style="margin:0 0 10px 0;font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#0f766e;">{{ $roleLabel }} Communication</p>
