    ...
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Create a new school account in Sanfaani Schools.
            </p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('admin.schools.store') }}" enctype="multipart/form-data" data-loading-text="Saving..." class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="logo" value="{{ old('logo') }}"
                               class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                        <p class="mt-1 text-xs text-gray-500">Optional external URL or existing public disk path.</p>
                        @error('logo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500">JPG, PNG, or WebP. Maximum 2MB. Upload replaces the logo path above.</p>
                        @error('logo_upload')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div>
                            ...
                            <select name="default_language"
                                ...
                                @foreach ($supportedLanguages as $code => $language)
                                    <option value="{{ $code }}" @selected(old('default_language', 'en') === $code)>{{ $language['label'] }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="responsive-form-grid gap-6">
                        ...
                        <div>
                            ...
                            <select name="subscription_status"
                                    class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900">
                                <option value="active" @selected(old('subscription_status') === 'active')>Active</option>
                                <option value="trial" @selected(old('subscription_status') === 'trial')>Trial</option>
                                <option value="expired" @selected(old('subscription_status') === 'expired')>Expired</option>
                            </select>
