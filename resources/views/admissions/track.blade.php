...
<section class="card">
    ...
    <p>Enter the application number and tracking token shown on your acknowledgement page.</p>
    @if($errors->any())<div class="error">{{ $errors->first() }}</div>@endif
    <form method="POST" action="{{ route('admissions.track.submit') }}" data-loading-text="Checking status..." class="grid" style="margin-top:24px">
        @csrf
        ...
        @if(config('admissions.guardian_tracking_fallback_enabled'))
            ...
            @endif
        @endif
        <div class="field full"><button class="button" type="submit" data-loading-text="Checking status...">Check status</button></div>
    </form>
</section>
@endsection
