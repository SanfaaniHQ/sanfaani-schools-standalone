            ...
            <div class="flex min-w-0 items-center gap-2">
                @if ($workspaceContexts->count() > 1)
                    <div x-data="{ open: false }" @keydown.escape.window="open = false" class="relative">
                        ...
                        <button type="button" @click="open = true; $nextTick(() => $refs.activeRole?.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }))" class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-brand-primary/30 bg-brand-primary text-white shadow-sm shadow-emerald-900/10 transition-all duration-200 ease-out hover:bg-brand-hover hover:shadow-md sm:hidden" aria-label="{{ __('ui.switch_role') }}" aria-haspopup="dialog" aria-controls="workspace-switcher-popup" :aria-expanded="open.toString()">
