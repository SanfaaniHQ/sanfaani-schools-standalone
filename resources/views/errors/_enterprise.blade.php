            ...
            <section class="w-full max-w-2xl rounded-lg border border-border-subtle bg-bg-secondary p-6 shadow-lg sm:p-8" aria-labelledby="error-title">
                <div class="flex flex-col gap-5 sm:flex-row sm:items-start">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="mt-3 break-words text-sm leading-6 text-text-secondary">{{ __('errors.help', ['email' => $supportEmail]) }}</p>

                        <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                            <a href="{{ $dashboardUrl }}" class="ui-button-primary w-full justify-center text-center sm:w-auto">{{ __('errors.return_dashboard') }}</a>
                            <button type="button" onclick="history.length > 1 ? history.back() : window.location.assign(@js($dashboardUrl))" class="ui-button-secondary w-full justify-center text-center sm:w-auto">
                                {{ __('errors.go_back') }}
                            </button>
                            <a href="{{ $homeUrl }}" class="ui-button-ghost w-full justify-center text-center sm:w-auto">{{ __('errors.go_home') }}</a>
                        </div>
                    </div>
                </div>
            </section>
